  ...
  receive(batch) {
    ...
    for (const [source, status, d1, d2] of batch) {
      ...
      if (command === 128 || (command === 144 && d2 === 0)) this.off(s, d1);
        ...
        this.engine.noteOn(id, d1, undefined, d2 / 127); this.apply(s);
      } else if (command === 224) { s.bend = d1 | (d2 << 7); this.apply(s); }
      else if (command === 192) this.program(d1);
      else if (command === 176) {
        if (d1 === 64) {
          s.pedal = d2 >= 64;
          ...
          s.bend = 8192; s.expression = 1; s.pedal = false; s.rpnMsb = s.rpnLsb = 127;
        ...
        else if (d1 === 74) this.engine.update({ cutoff: 80 * Math.pow(200, d2 / 127) });
        else if (d1 === 1) this.engine.update({ lfoDepth: d2 / 127 * 50 });
      }
    }
    this.changed();
  }
